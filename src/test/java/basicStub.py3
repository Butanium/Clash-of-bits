...
while True:
    ...
    for i in range(total_entities):
        # ent_id: the unique entity id, stay the same for the whole game
        # ent_type: the entity type in a string. It can be ALLY | ENEMY
        ...
        # health_rank: entities are sorted in ascendant order based on their amount of health, this is the rank of the current entity in the sorted list
    ...
    for i in range(ally_bot_alive):
        for j in range(total_entities):
            # ent_id: the unique entity id
            # ent_type: the entity type in a string. It can be SELF | ALLY | ENEMY
            ...
            # shield_comp: -1 if the entity has more shield than the current bot, 0 if it's equal, 1 if your bot as more shield
