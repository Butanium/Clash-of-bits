    ...
    <div class="statement-section statement-protocol">
        ...
        <div class="blk">
            ...
            <div class="text">
                ...
                <var>distMeRank</var>. For the first iteration, it sends its own information so <var>entType</var> is
                <const> "SELF"</const>
            ...
            <div class="blk">
                ...
                <div class="text">  One line containing all your orders separated by <const>";"</const> like this <const> "order1;order2;order3;..." </const>
                <br> An order has to follow the following synthax : <action>yourBotID [ACTION] [TARGETS]</action>
                <br><action>ACTION</action> has to be a valid action : <action>ATTACK</action>, <action>MOVE</action>, <action>FLEE</action>, <action>IDLE</action>

<action> TARGETS </action> has to use this synthax : <action>targetID1,targetID2,targetID3</action>
<br>
It has also to follow specifics rules depending on the action you try to perform :
<ul>
    <li>If the action is <action>ATTACK</action>, <action>TARGETS</action> has to contains <const>1</const> <strong> enemy bot </strong> id, not more, not less, don't try to attack yourself or your allies</li>
    <li>If the action is <action> MOVE </action> or <action> FLEE </action>, <action>TARGETS</action> has to contain at least 1 entity id</li>
    <li>If the action is <action>IDLE </action>, <action>TARGETS</action> can contain <strong> anything </strong> or just <strong> nothing</strong>, in any case it'll be ignored. <del> it allows you to REALLY make your bots thinking about the meaning of life </del></li>
</ul>If you send 2 differents action concerning the same bot you'll loose the game because your bot will run out of memory (┬┬﹏┬┬)
<br> If you don't send any input all your bots will <action> IDLE </action>. If you don't send any order concerning one or more of your bot, they will <action>IDLE</action>
